...
def get_list_of_university_towns():
    ...
    df['State'] = df[df['RegionName'].str.contains(r'\[edit\]')]
...
def get_recession_end():
    ...
    start_index = df[df['quarter']==start_quarter].index.tolist()[0]
    for i in range(start_index + 2, len(df)):
        if (df.iloc[i][1] > df.iloc[i-1][1]) and (df.iloc[i-1][1] > df.iloc[i-2][1]):
            return df.iloc[i][0]
    return ''
...
def get_recession_bottom():
    ...
    end_index = df[df['quarter']==end_quarter].index.tolist()[0]
    recession_bottom = start_index + 2
    min_gdp = df.iloc[recession_bottom][1]
    for i in range(start_index, end_index):
        if (df.iloc[i][1] < min_gdp):
            max_gdp = df.iloc[i][1]
            recession_bottom = i
